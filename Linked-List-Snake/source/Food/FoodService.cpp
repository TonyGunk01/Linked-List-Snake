...
namespace Food
{
	...
	{
		FoodItem* new_food = new FoodItem();
		new_food->initialize(position, cell_width, cell_height, type);
		return new_food;
	}
	...
	{
		std::uniform_int_distribution<int> dist_x(0, LevelModel::number_of_columns - 1);
		std::uniform_int_distribution<int> dist_y(0, LevelModel::number_of_rows - 1);

		int x_position = dist_x(random_engine);
		int y_position = dist_y(random_engine);
